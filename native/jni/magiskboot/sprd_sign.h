...
const uint8_t private_key_n[] = {
    ...
    179};
const uint8_t private_key_e[] = {1, 0, 1};
...
struct sprd_rsa_info {
  uint32_t bits;
  uint32_t e;
  uint8_t n[2048 >> 3];
} __attribute__((packed));

struct sprd_crypted_hash {
  uint32_t unknown1;
  struct sprd_rsa_info rsa_info;
  uint8_t hash[SHA256_DIGEST_LENGTH];
  uint32_t unknown2;
  uint32_t anti_rollback_version;
  uint8_t signature[2048 >> 3];
} __attribute__((packed));
